  ...
  const checkUserAccess = () => {
    ...
    if (userRequest) {
      if (userRequest.status === 'approved') {
        ...
        alert('✅ ДОСТУП ОДОБРЕН!\n\n🎉 Ваш запрос был одобрен администратором.\n📅 Дата одобрения: ' + (userRequest.approvedDate || 'Недавно') + '\n\n🔍 Теперь вы можете пользоваться поиском по каталогу!');
      } else if (userRequest.status === 'pending') {
        setAccessRequestSent(true);
        alert('⏳ ЗАПРОС В ОЖИДАНИИ\n\n📋 Ваш запрос отправлен администратору и ожидает рассмотрения.\n📅 Дата отправки: ' + userRequest.requestDate + '\n\n⏰ Пожалуйста, подождите решения администратора.');
      } else if (userRequest.status === 'rejected') {
        // Запрос отклонен - показать возможность отправить новый
        setAccessRequestSent(false);
        alert('❌ ЗАПРОС ОТКЛОНЕН\n\n🚫 Ваш запрос был отклонен администратором.\n📅 Дата отклонения: ' + (userRequest.approvedDate || 'Недавно') + '\n\n💡 Вы можете отправить новый запрос, если считаете это необходимым.');
      }
    } else {
      alert('📝 ЗАПРОС НЕ НАЙДЕН\n\n❓ У вас нет активных запросов на доступ к каталогу.\n\n💡 Нажмите "Отправить запрос на доступ" чтобы получить доступ к поиску.');
    }
